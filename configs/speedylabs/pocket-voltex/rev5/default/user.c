...
void CALLBACK_RGBCalculateNextFrame(void) {
  // Original controller uses a 768-wheel
  // Every 1ms, counter increments. Once at 32, hue increments.
  // 24.5s to advance through entire color wheel.

  // Scaling to our 256 wheel based on our frame rate, we should delay around 11 frames per hue increment.
  const uint8_t delay_per_increment =
    ((((768*32) / 1000) * RGB_FRAMERATE_TARGET) / 256);
  static uint8_t hue_incrementer = 0;
  hue_incrementer++;
  if (hue_incrementer < delay_per_increment) return;
