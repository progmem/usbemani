#include "usb/descriptors/konamicloud.ddr.h"
...
//  Default behavior:
//  * Buttons: L, R, Up, Down, Left, Right, two gaps, O, X
//  Pad:
//    L   R
//    X ^ O
//    <   >
...
WEAK void CALLBACK_OnKonamiCloudInputRequest(USB_InputReport_KonamiCloud_t *input) {
  // Prioritize FSRs first, then buttons
  // Preserve only an even number of FSRs
  const uint8_t  divider = (ANALOG_CHANNELS_ACTIVE & 0x0E);
  const uint16_t inputs  = (
    (Analog_GetAllDigital() & ((1 << divider) - 1)) |
    Button_GetAll() << divider;
  )

  // Prioritize arrows, then O/X, then L/R
  input->buttons |= (inputs & 0x0F) << 2;
  input->buttons |= (inputs & 0x30) << 4;
  input->buttons |= (inputs & 0xC0) >> 6;
}
